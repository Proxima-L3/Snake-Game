...
class BasicButton(object):
    ...
    def __init__(self, window, pos, text, font_size, width, height, start_sfx=''):  # if a start_sfx is desired only allow '_start_game' string as input
        self.window = window
        self.pos = pos
        self.text = text
        self.font_size = font_size
        ...
        self.start_sfx = start_sfx
        self.button_hover_sfx = pygame.mixer.Sound(os.path.join('project_assets/sfx', 'button_hover.wav'))
        self.button_click_sfx = pygame.mixer.Sound(os.path.join('project_assets/sfx', f'button_click{self.start_sfx}.wav'))
        self.hovered = False
        self.text_color = (10, 115, 205)
        self.font = pygame.font.Font('project_assets/fonts/bubble_pixelated.TTF', self.font_size)
        self.bg_image = pygame.transform.scale(pygame.Surface.convert_alpha(pygame.image.load(os.path.join('project_assets/buttons', 'stoneButtonReady.png'))), (self.width, self.height))
        self.bg_image_hover = pygame.transform.scale(pygame.Surface.convert_alpha(pygame.image.load(os.path.join('project_assets/buttons', 'stoneButtonHovered.png'))), (self.width, self.height))
        self.bg_image_clicked = pygame.transform.scale(pygame.Surface.convert_alpha(pygame.image.load(os.path.join('project_assets/buttons', 'stoneButtonPressed.png'))), (self.width, self.height))
        self.bg_rect = self.bg_image.get_rect(center=(self.pos[0], self.pos[1]))
        self.text_surface = self.font.render(self.text, True, self.text_color)
        self.text_rect = self.text_surface.get_rect(center=(self.pos[0], self.pos[1]))

    def is_hovering(self, mouse_pos):
        if mouse_pos[0] in range(self.bg_rect.left, self.bg_rect.right) and mouse_pos[1] in range(self.bg_rect.top, self.bg_rect.bottom):
            return True

    def draw_clicked(self, sfx_bool):
        self.window.blit(self.bg_image_clicked, self.bg_rect)
        self.window.blit(self.text_surface, self.text_rect)
        pygame.display.update()
        if sfx_bool == 'True':
            self.button_click_sfx.play()

    def draw(self, mouse_pos, sfx_bool):
        if self.is_hovering(mouse_pos):
            self.window.blit(self.bg_image_hover, self.bg_rect)
            self.window.blit(self.text_surface, self.text_rect)
            if self.hovered is False:
                if sfx_bool == 'True':
                    self.button_hover_sfx.play()
            ...
            self.window.blit(self.bg_image, self.bg_rect)
            self.window.blit(self.text_surface, self.text_rect)
            if self.hovered is True:
                self.hovered = False


class ApplyButton(BasicButton):

    def __init__(self, window, pos, text, font_size, width, height):
        super().__init__(window, pos, text, font_size, width, height)
        self.button_click_sfx = pygame.mixer.Sound(os.path.join('project_assets/sfx', 'button_apply_changes.wav'))


class ArrowButton(object):

    def __init__(self, window, pos, direction, width, height):  # only allow 'left' or 'right' to be the input for direction
        self.window = window
        self.pos = pos
        self.direction = direction
        self.width = width
        ...
        self.hovered = False
        self.button_hover_sfx = pygame.mixer.Sound(os.path.join('project_assets/sfx', 'button_hover.wav'))
        self.button_click_direction_sfx = pygame.mixer.Sound(os.path.join('project_assets/sfx', f'button_click_{self.direction}.wav'))
        self.arrow_image = pygame.transform.scale(pygame.Surface.convert_alpha(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonReady{self.direction.capitalize()}.png'))), (self.width, self.height))
        self.arrow_image_hover = pygame.transform.scale(pygame.Surface.convert_alpha(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonHovered{self.direction.capitalize()}.png'))), (self.width, self.height))
        self.arrow_image_clicked = pygame.transform.scale(pygame.Surface.convert_alpha(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonPressed{self.direction.capitalize()}.png'))), (self.width, self.height))
        self.arrow_rect = self.arrow_image.get_rect(center=self.pos)

    def is_hovering(self, mouse_pos):
        if mouse_pos[0] in range(self.arrow_rect.left, self.arrow_rect.right) and mouse_pos[1] in range(self.arrow_rect.top, self.arrow_rect.bottom):
            return True
...
class CutoutArrowButton(ArrowButton):
    ...
    def __init__(self, window, pos, direction, width, height):  # only allow 'left' or 'right' to be the input for direction
        ArrowButton.__init__(self, window, pos, direction, width, height)
        self.arrow_image = pygame.transform.scale(pygame.Surface.convert_alpha(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonCutoutReady{self.direction.capitalize()}.png'))), (self.width, self.height))
        self.arrow_image_hover = pygame.transform.scale(pygame.Surface.convert_alpha(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonCutoutHovered{self.direction.capitalize()}.png'))), (self.width, self.height))
        self.arrow_image_clicked = pygame.transform.scale(pygame.Surface.convert_alpha(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonCutoutPressed{self.direction.capitalize()}.png'))), (self.width, self.height))

    def update_button_pos(self, pos):
        self.pos = pos
        self.arrow_rect = self.arrow_image.get_rect(midleft=self.pos)
"""
...
class Button(object):
    ...
    def __init__(self, window, pos, text, button_hover_sfx, button_click_sfx):
        self.window = window
        self.pos = pos
        self.text = text
        self.button_hover_sfx = button_hover_sfx
        self.button_click_sfx = button_click_sfx
        self.hovered = False
        self.text_color = (10, 115, 205)
        self.font = pygame.font.Font('project_assets/fonts/bubble_pixelated.TTF', 26)
        self.width = 315
        self.height = 60
        self.bg_image = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', 'stoneButtonReady.png')), (self.width, self.height))
        self.bg_image_hover = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', 'stoneButtonHovered.png')), (self.width, self.height))
        self.bg_image_clicked = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', 'stoneButtonPressed.png')), (self.width, self.height))
        self.bg_rect = self.bg_image.get_rect(center=(self.pos[0], self.pos[1]))
        self.text_surface = self.font.render(self.text, True, self.text_color)
        self.text_rect = self.text_surface.get_rect(center=(self.pos[0], self.pos[1]))
...
class BackButton(object):
    ...
    def __init__(self, window, pos, text, button_hover_sfx, button_click_sfx):
        ...
        self.hovered = False
        self.text_color = (10, 115, 205)
        self.font = pygame.font.Font('project_assets/fonts/bubble_pixelated.TTF', 22)
        self.width = 115
        self.height = 35
        self.bg_image = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', 'stoneButtonReady.png')), (self.width, self.height))
        self.bg_image_hover = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', 'stoneButtonHovered.png')), (self.width, self.height))
...
class ArrowButton(object):
    ...
    def __init__(self, window, pos, direction, button_hover_sfx, button_click_direction_sfx):  # only allow 'left' or 'right' to be the input for direction
        self.window = window
        self.pos = pos
        self.direction = direction
        self.button_hover_sfx = button_hover_sfx
        self.button_click_direction_sfx = button_click_direction_sfx
        self.hovered = False
        self.width = 35
        self.height = 35
        self.arrow_image = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonReady{self.direction.capitalize()}.png')), (self.width, self.height))
        self.arrow_image_hover = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonHovered{self.direction.capitalize()}.png')), (self.width, self.height))
        self.arrow_image_clicked = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonPressed{self.direction.capitalize()}.png')), (self.width, self.height))
        self.arrow_rect = self.arrow_image.get_rect(center=(self.pos[0], self.pos[1]))
...
# game_options_screen
class CutoutArrowButton(object):

    def __init__(self, window, direction, button_hover_sfx, button_click_direction_sfx):  # only allow 'left' or 'right' to be the input for direction
        self.window = window
        # self.pos = pos
        self.direction = direction
        self.button_hover_sfx = button_hover_sfx
        ...
        self.width = 32
        self.height = 28.9
        self.arrow_image = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonCutoutReady{self.direction.capitalize()}.png')), (self.width, self.height))
        self.arrow_image_hover = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonCutoutHovered{self.direction.capitalize()}.png')), (self.width, self.height))
        self.arrow_image_clicked = pygame.transform.scale(pygame.image.load(os.path.join('project_assets/buttons', f'ArrowButtonCutoutPressed{self.direction.capitalize()}.png')), (self.width, self.height))
        # self.arrow_rect = self.arrow_image.get_rect(midleft=self.pos)

    def is_hovering(self, mouse_pos, pos):
        if mouse_pos[0] in range(self.arrow_image.get_rect(midleft=pos).left, self.arrow_image.get_rect(midleft=pos).right) and mouse_pos[1] in range(self.arrow_image.get_rect(midleft=pos).top, self.arrow_image.get_rect(midleft=pos).bottom):
            return True

    def draw_clicked(self, pos, sfx_bool):
        self.window.blit(self.arrow_image_clicked, self.arrow_image.get_rect(midleft=pos))
        pygame.display.update()
        if sfx_bool == 'True':
            self.button_click_direction_sfx.play()

    def draw(self, mouse_pos, pos, sfx_bool):
        if self.is_hovering(mouse_pos, pos):
            self.window.blit(self.arrow_image_hover, self.arrow_image.get_rect(midleft=pos))
            if self.hovered is False:
                if sfx_bool == 'True':
                    self.button_hover_sfx.play()
                self.hovered = True
        else:
            self.window.blit(self.arrow_image, self.arrow_image.get_rect(midleft=pos))
            if self.hovered is True:
                self.hovered = False
...
class ApplyButton(object):

    def __init__(self, window, pos, text, button_hover_sfx, button_apply_changes_sfx):
        self.window = window
        self.pos = pos
        self.text = text
        self.button_hover_sfx = button_hover_sfx
        self.button_apply_changes_sfx = button_apply_changes_sfx
        self.hovered = False
        self.text_color = (10, 115, 205)
        self.font = pygame.font.Font('project_assets/fonts/bubble_pixelated.TTF', 22)
        self.width = 115
    ...
    def draw_clicked(self, sfx_bool):
        ...
        pygame.display.update()
        if sfx_bool == 'True':
            self.button_apply_changes_sfx.play()
